...
app.post('/sciask_api/register.php', (req, res) => {
  try {
    ...
    const stmt = db.prepare('INSERT INTO users (name, email, phone, university, faculty, tracks, password, points, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
    const result = stmt.run(name, email, phone || '', university || '', faculty || '', tracksStr, password, 0, 'student');
